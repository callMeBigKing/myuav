...
class UAV:
    ...
    def decision(self):
        ...
        # 暂时用前序遍历算法 处理一下
        path = []
        path.append(tuple(self.site))
        best_path = []  # 用来存储最佳路径
        best_value = -np.inf  # 用来存储最佳value
    ...
    def get_path_value(self, path):
        ...
        for i, site in zip(range(search_step), path):
            ...
            pro_value = 0
            for j in range(Nt):
                if self.detect_flag[j] == 0:  # 不需要继续搜索的目标直接跳过
                    continue
...
#             if new_u_y >= Ny * Ly:
#                 add_x = (Ny * Ly - u_y) / np.tan(np.abs(np.pi - phi))
#                 new_u_y = Ny * Ly-0.1 # 不能等于边界，不然后面向下去整不好用
#                 new_u_x = u_x - add_x
#         else:
#             add_y = (Nx * Lx - u_x) * np.tan(phi)
#             new_u_y = u_y + add_y
